...
% NNTRAIN обучает нейронную сеть [nn, L] = nnff (nn, x, y, opts)

% обучает нейронную сеть nn с входом x и выходом y для эпох opts.numepochs
% с мини-пакетами размера opts.batchsize. Возвращает нейронную сеть nn с 
% обновленными активациями, ошибками, весами и смещениями, 
% (nn.a, nn.e, nn.W, nn.b) и L, квадратом суммарной ошибки для каждой
% обучающей мини-партии.
...
for i = 1 : numepochs
    ...
    if useGpu
        ...
        % selThese are the standard deviations of  the dynamics distribution, that is how much we expect the target object might move from one frame to the next.  The meaning of each
